...
(function(){
    ...
    function convert_url(url){
        ...
        url = utils.trim(url)
			...
			//.replace(/tv\.sohu\.com\/[\w]+\/([\w]+)\.shtml.*$/i, "share.vrs.sohu.com/my/v.swf&id=$1");
    ...
    function createPreviewVideo(url){
        ...
        $G("preview").innerHTML = '<div class="previewMsg"><span  id="embed-load-tips">正在检测中...</span></div>'+
        '<embed id="embed" name="embed" class="previewVideo" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
            ' src="' + conUrl + '"' +
            ' width="' + 420  + '"' +
            ' height="' + 280  + '"' +
        ...
        var loadingObj = window['embed'],
            tipsObj = document.getElementById('embed-load-tips'),
            now = new Date().getTime(),
            timer = null;

        var _load = function () {

          if( loadingObj.PercentLoaded && Math.floor(loadingObj.PercentLoaded()) >= 100 ){
			setTimeout(function () {
			  tipsObj.innerHTML = '<span style="color:green;">检测通过</span>';
			},1000);
          }else{
            ...
            if( new Date().getTime() - now > 10000 ){
              tipsObj.innerHTML = '<span style="color:red;">检测错误</span>';
